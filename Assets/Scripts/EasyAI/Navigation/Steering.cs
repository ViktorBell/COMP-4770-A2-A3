...
namespace EasyAI.Navigation
{
    ...
    public static class Steering
    {
        ...
        {
            // Calculate the desired velocity to move away from the pursuer
    ...
    {
        // Calculate the future position of the evader
        Vector2 evaderVelocity = (evader - evaderLastPosition) / deltaTime;
        Vector2 futurePosition = evader + evaderVelocity * deltaTime;

        // Seek the future position of the evader
        Vector2 desiredVelocity = (futurePosition - position).normalized * speed;
        Vector2 steeringForce = desiredVelocity - velocity;
        ...
        //An attempt to create a pursue function that took agents as agrguments but it didn't quite work
        /*
        private static Vector2 PursueAgent(Agent pursuingAgent, Agent evadingAgent, float deltaTime) {


            double distanceToTarget = (pursuingAgent.transform.position - evadingAgent.transform.position).magnitude;//(position - evaderLastPosition).magnitude;
            double speedFactor = pursuingAgent.MoveSpeed + evadingAgent.MoveVelocity.magnitude;
            double lookAheadTime = (distanceToTarget / speedFactor) * deltaTime;

            Vector2 pursuitVelocityVector = evadingAgent.transform.position + evadingAgent.MoveVelocity



                return pursuitVelocityVector;
        }
        */
        ...
        {
            // Calculate the future position of the pursuer
            Vector2 pursuerVelocity = (pursuer - pursuerLastPosition) / deltaTime;
            Vector2 futurePosition = pursuer + pursuerVelocity * deltaTime;

            // Flee from the future position of the pursuer
            Vector2 desiredVelocity = (position - futurePosition).normalized * speed;
            Vector2 steeringForce = desiredVelocity - velocity;
